...
/**
 * Created by supc on 2018/2/26 0026.
 */
public class TwoWayANOVA {
    ...
    private static final Logger log = Logger.getLogger(TwoWayANOVA.class);

    private double[] meanXi;
    private double[] meanXj;
    ...
    private double FCriticalR;
    private double FCriticalC;
    ...
    * 如果返回false表明不存在显著差异
    */
    public boolean m_TwoWayANOVA(double[][] dataSet) throws Exception{
        log.info("双因素方差方法被调用");
        ...
        meanXi = new double[nRow];
        double[] tmp = new double[nColumn];
        for (int i = 0; i < nRow; i++) {
            tmp = dataSet[i];
            meanXi[i] = Statistics.getAverage(tmp);
        }

        meanXj = new double[nColumn];
        tmp = new double[nRow];
        for (int j = 0; j < nColumn; j++) {
            for (int i = 0; i < nRow; i++) {
                tmp[i] = dataSet[i][j];
            }
            meanXj[j] = Statistics.getAverage(tmp);
        }
        double sum = 0;
        for (int i = 0; i < nRow; i++) {
            for (int j = 0; j < nColumn; j++)
        ...
        for (int i = 0; i < nRow; i++) {
            for (int j = 0; j < nColumn; j++) {
                SST += Math.pow((dataSet[i][j] - meanX), 2);
                SSE += Math.pow((dataSet[i][j] - meanXi[i] - meanXj[j] + meanX), 2);
                SSR += Math.pow((meanXi[i] - meanX), 2);
                SSC += Math.pow((meanXj[j] - meanX), 2);
            }
        }
        ...
        MSC = SSC / (r - 1);
        MSE = SSE / ((k - 1) * (r - 1));
        if ((k - 1) * (r - 1) > 100 || (k-1)>10 || (r-1)>10) {
            log.error("使用双因素方差分析时，数据超出查表范围，请降低数据的数量");
            throw new Exception("使用双因素方差分析时，数据超出查表范围，请降低数据的数量");
        }

        Fr = MSR / MSE;
        FCriticalR = criticalTable[(k - 1) * (r - 1) - 1][k - 1 - 1];

        Fc = MSC / MSE;
        FCriticalC = criticalTable[(k - 1) * (r - 1) - 1][r - 1 - 1];
        if (Fr > FCriticalR || Fc > FCriticalC){
            return true;
        }
        return false;
    }
    ...
    public double getFCriticalR() {
        return FCriticalR;
    }

    public double getFCriticalC() {
        return FCriticalC;
    }
}
